...
```
**补充**：scores 张量通常确实有三个维度，它们分别代表
​​* 第0维​​：批次维度​​：代表一次同时处理多少个独立的样本。例如，一个批次有8个句子，这个维度大小就是8。每个句子的计算是独立的。
* ​​第1维​​：​​查询序列长度​​	：代表每个样本中，需要生成输出的位置数量。例如，一个句子有10个词，这个维度大小就是10。每个词都会产生一个查询。
* ​​第2维​​	​：键序列长度​​：代表每个样本中，可供查询和参考的位置数量。通常与查询序列长度相同。每个键都会被查询。
...
attention(x, x, x)
```
**理解**：将​​同一个​​输入张量 x 同时作为查询、键和值的来源，传入注意力计算函数。这意味着序列中的每个元素，既要用作查询来提问，也要用作键来被匹配，还要用作值来提供信息

### 2.1.5 掩码自注意力

掩码自注意力(Mask Self-Attention)就像做预测时给模型挡掉句子里还没到的词（token），让它只能用前面的内容猜下一个词，不偷看也能高效练习。
...
从理论上讲，若训练语料的规模足够大，通过 “依据前文 token 逐次预测下一个 token 以补全文本序列” 的学习过程，模型可掌握任意文本序列的建模逻辑，实现对各类文本的补全任务。但需注意，这种逐步骤（先完成 Step 1 再进行 Step 2，依次推进至序列结束）的学习模式属于**串行过程** —— 而**并行计算**本就是 Transformer 相较于 RNN 的核心优势之一，其设计初衷就是通过并行提升计算效率。若训练时对每一份语料都需按串行方式完成序列补全才能让模型学习，就会完全丧失 Transformer 的并行优势，导致计算效率大幅降低，与 Transformer 的架构设计目标相悖。
